...
- **ハッシュタグ**: **最大2件まで**。
- **スマート空き枠検索 (Smart Slot Finding)** (v4.10): 
    - 従来の「日時指定」や「自動補充」は廃止し、**「指定された本数」を「空いている枠」に埋める** ロジックへ統一。
    - 現在時刻以降の標準投稿枠（08:00, 12:00, 20:00）をスキャンし、予約済みの枠をスキップして、空き枠が見つかるまで未来へ探索を続ける。
- **多様性確保 (Diversity Rule)** (v4.11):
    - **ペルソナローテーション**: 「科学者」「友人」「コーチ」の3つの視点を強制的にローテーションさせ、文体と構成を変化させる。
    - **禁止事項**: 同じ導入文（フック）や汎用的な挨拶（Hello everyone等）のAI生成をシステムレベルで禁止。
- **緊急即時生成 (Emergency Failover)** (v3.7): 
    - 投稿実行時に該当枠の予約が0件だった場合、その場でAIが1記事を生成し、即座に投稿を完了させる。
    - 10分ごとの頻繁なCron実行時でも重複生成を防ぐ hourly ロック機構を搭載。
- **管理画面の一本化 (v7.1 New)**:
    - **UI構造**: 管理画面 (`admin.html`) は、**LPリポジトリ (`af-mini-specialLP`) のみに存在し、管理される**。
    - **入力仕様**: 生成指示は「日数(Days)」ではなく**「生成記事数(Articles)」**を指定する (v4.9)。
