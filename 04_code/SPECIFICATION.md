...
### 2.3 管理画面 UIガバナンス (Admin UI Governance - v7.6 黄金レイアウト)
管理画面は「一瞥ですべての戦況を把握する」司令塔であり、デザイン変更（リニューアル）時も以下の物理的構造を**聖域として固定**し、絶対に破壊してはならない。

1. **3段垂直構成 (Three-Tier Stack)**:
    - **上段 [h-[28%]]**: 運用タクティクス（X予約・履歴）。クイックアクションと配信表が横に並ぶ。
    - **中段 [grid/h-[15%前後の可変高度]]**: 戦略インテリジェンス（LPパフォーマンスカード）。5列固定のグリッド形式。
    - **下段 [残り全高]**: 詳細分析。左にKPI縦パネル(15%)、中央に回遊フロー、右に実行ログを配置。
2. **スクロール・ゼロ原則**:
    - ブラウザのデフォルトスクロールバーを発生させない。常に `h-screen` 内に収まるように padding や余白を計算すること。
    - 各セクション内の溢れは個別の `overflow-y-auto`（スクロール隠し）で処理する。
3. **ID・データ紐付けの固定**:
    - `kpi-clicks`, `kpi-cv`, `kpi-revenue`, `kpi-cvr` などのID名は、バックエンドからの流し込みと直結しているため、タグのリニューアル時も**名称を一切変えない**。
    - `render()` 関数内で定義されているデータマッピング構造（どの変数がどのIDに刺さるか）を優先し、UIはその定義に合わせる。
4. **情報の「高効率圧縮」**:
    - デザインを美しくする際も、文字サイズを大きくして情報の密度を下げる行為（見切れの発生）は禁止。
    - 常に「全LPの数字が一度に見えること」を最優先する。
