...
## [v4.0] - 2026-02-16
### Added
- **トラッキング3分類（Human/Dev/Bot）機能**: アクセス元を「一般ユーザー (Human)」「管理者 (Dev)」「機械的アクセス (Bot)」に自動判別し、個別にフラグ管理。
- **管理者トラッキング表示**: 管理画面右上に内部用の `DEV` / `BOT` カウントを表示し、システム稼働を視覚化。

### Changed
- **統計データのノイズ除去**: メインのPV・CV・CVR集計からBotおよびDevアクセスを完全に除外。マーケティング判断を乱すノイズを排除。
- **LP IDの正規化**: 旧ID（`mini_main`）と新ID（`mini_lp`）を自動的に名寄せして「MAIN」カードへ統合。
