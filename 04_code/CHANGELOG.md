...
## [v6.5] - 2026-02-28
### Added
- **計測の聖域化 (Analytics Sanctuary)**:
  - `src/services/data_service.js` 内の主要計測メソッド（`addEventLog`, `isBot`, `getIpHash`）を「不可侵の聖域」として定義し、AIによる削除・改変を禁止。
  - 修正後に計測ロジックの生存を確認する検証スクリプト `verify_tracking.js` を導入。
- **ガバナンス強化 (Governance Update)**:
  - [GOVERNANCE.md](./GOVERNANCE.md) に「計測ロジックの保護」条項を追加。

### Fixed
- **計測機能の完全復旧 (Analytics Restoration)**:
  - 消失していたリダイレクト時のログ記録機能を再実装。クリック、PV、コンバージョンがリアルタイムでスプレッドシートに反映されるように修復。
  - スプレッドシート上の既存ヘッダー「記録日時」をダッシュボードが正しく認識できるよう、タイムスタンプ処理をマルチヘッダー対応に強化。
